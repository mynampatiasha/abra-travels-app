...
class AdminLiveLocationWholeVehicles extends StatefulWidget {
  const AdminLiveLocationWholeVehicles({super.key});
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          final vehiclesData =
              data['data']['vehicles'] as List<dynamic>? ?? [];
          final alertsData =
              data['data']['alerts'] as Map<String, dynamic>? ?? {};
          final summaryData =
          ...
          child: Column(
            children: [
              FloatingActionButton.small(
                heroTag: 'toggle_list',
                onPressed: () =>
                    setState(() => _showVehicleList = !_showVehicleList),
        ...
        child: Row(
          children: [
            _buildStatItem('Total', _stats['total']?.toString() ?? '0',
                Colors.blue),
            _buildStatItem('Active', _stats['active']?.toString() ?? '0',
                Colors.green),
            _buildStatItem(
                'Idle', _stats['idle']?.toString() ?? '0', Colors.orange),
            _buildStatItem('Offline',
                _stats['offline']?.toString() ?? '0', Colors.red),
            _buildStatItem('Completed',
                _stats['completed']?.toString() ?? '0', Colors.grey),
          ],
        ),
      ),
    );
