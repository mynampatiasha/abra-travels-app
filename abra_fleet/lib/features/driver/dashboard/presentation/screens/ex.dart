...
class _ProfileDriverPageState extends State<ProfileDriverPage> {
  late Future<DocumentSnapshot<Map<String, dynamic>>> _driverProfileFuture;
  ...
  void _loadInitialData() {
    ...
    _driverProfileFuture.then((driverDoc) {
      if (driverDoc.exists) {
        setState(() {
          _documentStatusFuture = _documentService.getDocumentStatus(driverDoc.id);
        });
      }
    });
  }
  ...
  Future<DocumentSnapshot<Map<String, dynamic>>> _fetchDriverProfile() async {
      ...
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('jwt_token');
      if (token == null || token.isEmpty) throw Exception('No authenticated user found.');
  ...
  Future<void> _updatePhone(String driverDocId) async {
    if (!(_phoneFormKey.currentState?.validate() ?? false)) {
      return;
    }

    try {
      await FirebaseFirestore.instance.collection('users').doc(driverDocId).update({
  ...
  Future<void> _updateName(String driverDocId) async {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          final driverDocId = snapshot.data!.id;
              ...
              Row(
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: () => _updateName(driverId),
                  ),
                  IconButton(
  ...
  Widget _buildDriverInfoCard(Map<String, dynamic> data, String driverDocId) {
    ...
    return _buildCard(
      ...
      child: Column(
        children: [
          _buildProfileItem(label: 'Name', value: name.isEmpty ? 'N/A' : name),
          _buildProfileItem(label: 'Email', value: email),
          _buildEditablePhoneItem(
            label: 'Phone',
            value: phone,
  ...
  Widget _buildEditablePhoneItem({
                ...
                GestureDetector(
                  onTap: () {
                    setState(() {
                      ...
                      _phoneController.text = value != 'N/A' ? value : '';
                    });
                  },
                  child: const Icon(
                    Icons.edit,
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () => _updatePhone(driverDocId),
...
Future<bool> _submitIssueReport(String issue) async {
  try {
    ...
    await FirebaseFirestore.instance.collection('support_issues').add({
      'driverId': user.uid,
      'email': user.email,
      'issue': issue,
...
// ============== WIDGET AND SERVICE CLASSES (MOVED TO TOP-LEVEL) ==============
...
class DocumentService {
  ...
  Future<Map<String, DocumentStatus>> getDocumentStatus(String driverId) async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/status/$driverId'));
  ...
  Future<String> uploadDocument(
    ...
    try {
      ...
      final uri = Uri.parse('$_baseUrl/$endpoint/$driverId');
