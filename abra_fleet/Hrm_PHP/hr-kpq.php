...
error_log("=== HR-KPQ AUTH DEBUG ===");
error_log("GET user_email: " . ($currentUserEmail ? $currentUserEmail : 'EMPTY'));
error_log("SESSION user_name: " . ($currentUserName ? $currentUserName : 'EMPTY'));
...
if (!empty($currentUserEmail)) {
    error_log("Trying email lookup: $currentUserEmail");
    ...
    if ($res && mysqli_num_rows($res) > 0) {
        ...
        $currentUserName = $employee_name; // Update current user name
        error_log("✅ Email lookup SUCCESS: $employee_name ($employee_id) - $employee_department / $employee_position");
    } else {
        error_log("❌ Email lookup FAILED for: $currentUserEmail");
    }
}

// ✅ FALLBACK: Try session-based lookup (traditional PHP session)
if (empty($employee_id) && !empty($currentUserName)) {
    error_log("Trying session name lookup: $currentUserName");
    ...
    if ($res && mysqli_num_rows($res) > 0) {
        ...
        $employee_department = !empty($row['department']) ? $row['department'] : 'General';
        $employee_position = !empty($row['position']) ? $row['position'] : 'General';
        error_log("✅ Session name lookup SUCCESS: $employee_name ($employee_id) - $employee_department / $employee_position");
    } else {
        error_log("❌ Session name lookup FAILED for: $currentUserName");
    }
}

// ✅ FINAL FALLBACK: If still empty, use default admin
if (empty($employee_id)) {
    error_log("⚠️ No user found, using default admin fallback");
    $res = mysqli_query($con, "SELECT employee_id, name, department, position FROM hr_employees WHERE (email = 'admin@abrafleet.com' OR employee_id = 'ABRA001') AND (status = 'Active' OR status = 'active') LIMIT 1");
    if ($res && mysqli_num_rows($res) > 0) {
        $row = mysqli_fetch_assoc($res);
        $employee_id = $row['employee_id'];
        $employee_name = $row['name'];
        $employee_department = !empty($row['department']) ? $row['department'] : 'Management';
        $employee_position = !empty($row['position']) ? $row['position'] : 'Managing Director';
        $currentUserName = $employee_name;
        error_log("✅ Default admin fallback: $employee_name ($employee_id)");
    }
}

error_log("FINAL AUTH RESULT: employee_id=$employee_id, name=$employee_name, dept=$employee_department, pos=$employee_position");
...
// ✅ FIXED: More flexible matching for HR department variations
...
// ✅ IMPROVED: Check if user is HR Manager (any level) - more flexible matching
...
// ✅ UPDATED: Final access check includes HR managers
$hasAccess = ($is_management_dept || $is_hr_dept || $is_managing_director || $is_hr_manager || $is_abishek || $is_keerthi);

// 🔍 DEBUG PERMISSION CHECK
error_log("=== PERMISSION CHECK ===");
error_log("employee_name: $employee_name");
error_log("employee_department: $employee_department");
error_log("employee_position: $employee_position");
error_log("dept_normalized: $dept_normalized");
error_log("pos_normalized: $pos_normalized");
error_log("is_management_dept: " . ($is_management_dept ? 'YES' : 'NO'));
error_log("is_hr_dept: " . ($is_hr_dept ? 'YES' : 'NO'));
error_log("is_managing_director: " . ($is_managing_director ? 'YES' : 'NO'));
error_log("is_hr_manager: " . ($is_hr_manager ? 'YES' : 'NO'));
error_log("is_abishek: " . ($is_abishek ? 'YES' : 'NO'));
error_log("is_keerthi: " . ($is_keerthi ? 'YES' : 'NO'));
error_log("hasAccess: " . ($hasAccess ? 'YES' : 'NO'));
error_log("=== END PERMISSION CHECK ===");
...
if(!$hasAccess) {
    error_log("❌ ACCESS DENIED for user: $employee_name (ID: $employee_id)");
    die("Access Denied: User '" . htmlspecialchars($employee_name ? $employee_name : $currentUserName) . "' does not have permissions to access this page.<br><br>
    <strong>Access Requirements:</strong><br>
    - Management Department<br>
    - Human Resources Department<br>
    - Managing Director position<br><br>
    <strong>Your Details:</strong><br>
    - Name: " . htmlspecialchars($employee_name) . "<br>
    - Department: " . htmlspecialchars($employee_department) . "<br>
    - Position: " . htmlspecialchars($employee_position) . "<br><br>
