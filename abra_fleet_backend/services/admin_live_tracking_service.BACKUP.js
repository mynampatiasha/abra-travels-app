...
async function fetchLiveVehicles(db, date, statusFilter, companyFilter) {
  try {
    console.log(`\n🔍 fetchLiveVehicles — date=${date} status=${JSON.stringify(statusFilter)}`);

    // Strict date window for ISODate collections (IST is UTC+5:30)
    // We use full calendar day in UTC to catch all Indian timezone trips
    const dayStart = new Date(`${date}T00:00:00.000Z`);
    const dayEnd   = new Date(`${date}T23:59:59.999Z`);

    // ── 1. roster-assigned-trips ────────────────────────────────────────────
    // scheduledDate is a plain string "YYYY-MM-DD" — exact match ensures
    // stale trips from previous dates (e.g. Feb 17) are NEVER returned.
    const rosterQuery = {
      scheduledDate: date,          // ← STRICT: must equal today's date string
      status: statusFilter,
    };

    if (companyFilter && companyFilter !== 'all') {
      rosterQuery['stops.customer.email'] = new RegExp(`@${companyFilter}\\.`, 'i');
    }
    ...
    console.log(`  roster-assigned-trips : ${rosterTrips.length} (date=${date})`);
    ...
    for (const v of [
      ...rosterTrips.map(normaliseRosterTrip),
      ...manualTrips.map(normaliseManualTrip),
      ...clientTrips.map(normaliseClientTrip),
    ]) {
  ...
  for (const vehicle of vehicles) {
    try {
      ...
      if (!vehicle.currentLocation?.timestamp) {
        alerts.offline.push({
          vehicleId: vehicle.vehicleId, vehicleNumber: vehicle.vehicleNumber,
          driverName: vehicle.driverName, tripId: vehicle.tripId,
          source: vehicle.source, offlineSince: vehicle.lastUpdated || null,
          duration: 'Unknown',
        });
        await notifyVehicleOffline(db, vehicle, 'No GPS data');
      } else {
        ...
        if (lastUpdate < fiveMinutesAgo) {
          ...
          alerts.offline.push({
            vehicleId: vehicle.vehicleId, vehicleNumber: vehicle.vehicleNumber,
            driverName: vehicle.driverName, tripId: vehicle.tripId,
            source: vehicle.source, offlineSince: lastUpdate, duration,
          });
          await notifyVehicleOffline(db, vehicle, duration);
        }
      }
      ...
      if (vehicle.currentStop?.location?.coordinates?.latitude) {
        ...
        if (dist > 5) {
          alerts.routeDeviation.push({
            vehicleId: vehicle.vehicleId, vehicleNumber: vehicle.vehicleNumber,
            driverName: vehicle.driverName, tripId: vehicle.tripId,
            source: vehicle.source, deviation: dist.toFixed(2),
            currentLocation: vehicle.currentLocation,
            nextStop: vehicle.currentStop.customer?.name || 'Next stop',
          });
      ...
      if (vehicle.currentLocation.speed > 80) {
        alerts.speeding.push({
          vehicleId: vehicle.vehicleId, vehicleNumber: vehicle.vehicleNumber,
          driverName: vehicle.driverName, tripId: vehicle.tripId,
          source: vehicle.source, speed: vehicle.currentLocation.speed,
          location: { latitude: vehicle.currentLocation.latitude, longitude: vehicle.currentLocation.longitude },
        });
        await notifySpeedAlert(db, vehicle, vehicle.currentLocation.speed);
      }
...
async function getFleetStatistics(db, date) {
  try {
    ...
    const [rosterTrips, manualTrips, clientTrips] = await Promise.all([
      ...
      db.collection('trips').find({ scheduledPickupTime: { $gte: dayStart, $lte: dayEnd } }).toArray(),
      db.collection('client_created_trips').find({ scheduledPickupTime: { $gte: dayStart, $lte: dayEnd } }).toArray(),
    ]);
