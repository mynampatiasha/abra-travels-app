...
async function restoreRosters() {
  ...
  try {
    ...
    // Get customer UIDs from MongoDB users collection
    console.log('\n🔍 Getting customer data from MongoDB...\n');
    
    const rajesh = await db.collection('users').findOne({ email: 'rajesh.kumar@infosys.com' });
    const priya = await db.collection('users').findOne({ email: 'priya.sharma@infosys.com' });
    const amit = await db.collection('users').findOne({ email: 'amit.patel@infosys.com' });
    
    if (!rajesh || !priya || !amit) {
      console.log('❌ Could not find all 3 customers in database!');
    ...
    console.log(`✅ Rajesh Kumar UID: ${rajesh.uid}`);
    console.log(`✅ Priya Sharma UID: ${priya.uid}`);
    console.log(`✅ Amit Patel UID: ${amit.uid}`);
    ...
    const rostersToRestore = [
      {
        customerUid: rajesh.uid,
        customerName: rajesh.name || 'Rajesh Kumar',
        customerEmail: rajesh.email,
        customerPhone: rajesh.phone || '+91 9876543210',
        pickupLocation: 'Electronic City, Bangalore',
        pickupLatitude: 12.8456,
        pickupLongitude: 77.6603,
      },
      {
        customerUid: priya.uid,
        customerName: priya.name || 'Priya Sharma',
        customerEmail: priya.email,
        customerPhone: priya.phone || '+91 9876543211',
        pickupLocation: 'Whitefield, Bangalore',
        pickupLatitude: 12.9698,
        pickupLongitude: 77.7500,
      },
      {
        customerUid: amit.uid,
        customerName: amit.name || 'Amit Patel',
        customerEmail: amit.email,
        customerPhone: amit.phone || '+91 9876543212',
