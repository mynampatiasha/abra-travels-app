...
router.post('/:id/location', async (req, res) => {
    try {
        ...
        const ref = db.ref(`sos_events/${id}/gps`);
...
router.put('/:id/status', async (req, res) => {
    try {
        ...
        // Update in Firebase
        const db = admin.database();
        const ref = db.ref(`sos_events/${id}`);
        ...
        if (adminNotes) {
            firebaseUpdate.adminNotes = adminNotes;
        }

        await ref.update(firebaseUpdate);
        console.log(`[SOS Status Update] Event updated in Firebase.`);
...
router.put('/:id/resolve', async (req, res) => {
    try {
        ...
        // Add optional fields to Firebase update
        if (adminNotes) {
            firebaseUpdateData.adminNotes = adminNotes;
        }
        if (resolvedBy) {
            firebaseUpdateData.resolvedBy = resolvedBy;
        }
        if (resolutionType) {
            firebaseUpdateData.resolutionType = resolutionType;
        }

        await ref.update(firebaseUpdateData);
        console.log(`[SOS Resolve] Event updated in Firebase.`);
...
router.get('/', async (req, res) => {
    try {
        ...
        console.log(`[SOS Get] Fetching SOS events. Status filter: ${status || 'all'}, Organization: ${organizationDomain || 'all'}`);
        ...
        // Filter by organization domain if provided
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const ref = db.ref(`sos_events/${id}`);
        await ref.set(null);
        console.log(`[SOS Delete] Event removed from Firebase.`);
...
router.post('/resolve', (req, res, next) => {
    ...
    try {
        ...
        // Update Firebase
        const db = admin.database();
        const ref = db.ref(`sos_events/${sosId}`);
        await ref.update({
            status: 'Resolved',
            resolution: {
                photoUrl: photoUrl,
        ...
        console.log(`✅ [SOS Resolve] Firebase updated`);
        console.log(`\n✅ ============================================`);
 ...
 * @access  Public (add admin auth as needed)
 */
router.put('/:id/resolve-with-proof', async (req, res) => {
    try {
        const { id } = req.params;
        const { status, adminNotes, resolution } = req.body;
        
        console.log(`\n🔍 ============================================`);
        console.log(`🔍 [SOS Resolve with Proof] ID: ${id}`);
        console.log(`🔍 ============================================\n`);
        ...
        if (!resolution || !resolution.photoUrl || !resolution.notes) {
            return res.status(400).json({ 
                status: 'error', 
                message: 'Resolution proof (photo and notes) required.' 
            });
        }

        console.log(`📸 Photo URL: ${resolution.photoUrl}`);
        console.log(`📝 Notes: ${resolution.notes.substring(0, 50)}...`);
        ...
        if (resolution.latitude && resolution.longitude) {
            console.log(`📍 Location: ${resolution.latitude}, ${resolution.longitude}`);
        }
        ...
        console.log(`✅ [SOS Resolve with Proof] MongoDB updated`);
        ...
        console.log(`✅ [SOS Resolve with Proof] Firebase updated`);
        console.log(`\n✅ ============================================`);
        console.log(`✅ [SOS Resolve with Proof] COMPLETED`);
        console.log(`✅ ============================================\n`);

        res.status(200).json({ 
            status: 'success', 
            message: 'SOS resolved with proof successfully.',
            eventId: id
        });

    } catch (error) {
        console.error('\n❌ ============================================');
        console.error('❌ [SOS Resolve with Proof] ERROR:', error);
        console.error('❌ ============================================\n');
        res.status(500).json({ status: 'error', message: 'Internal Server Error' });
    }
});
