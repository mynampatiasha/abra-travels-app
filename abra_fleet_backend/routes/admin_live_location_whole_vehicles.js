...
router.get('/vehicles', verifyToken, async (req, res) => {
  try {
    ...
    // ========================================================================
    // STEP 2: Pass status filter as plain string (service handles in-memory filtering)
    // ========================================================================
    const statusFilter = status || 'all';
    
    // ========================================================================
    // STEP 3: Fetch live vehicles from service (returns { vehicles, summary })
    // ========================================================================
    const result = await liveTrackingService.fetchLiveVehicles(
      req.db,
      queryDate,
      statusFilter,
      company
    ...
    console.log(`✅ Found ${vehicles.length} vehicle(s) (filtered)`);
    console.log(`📊 Summary: total=${summary.total}, active=${summary.active}, idle=${summary.idle}, assigned=${summary.assigned}, unassigned=${summary.unassigned}, completed=${summary.completed}`);
    ...
    // ========================================================================
    // STEP 5: Return response with summary counts (always full fleet)
    // ========================================================================
    res.json({
      success: true,
