  ...
  try {
    ...
    if (customer.primaryEmail) {
      try {
        ...
        // Store reset token in customer record
        customer.passwordResetToken = resetToken;
        customer.passwordResetExpires = resetTokenExpiry;
        await customer.save();
  ...
  try {
    ...
    for (let i = 0; i < customers.length; i++) {
      try {
        ...
        if (customer.primaryEmail) {
          try {
            console.log(`   📧 Sending welcome email to: ${customer.primaryEmail}`);
            ...
            emailsSent++;
            console.log(`   ✅ Email sent to: ${customer.primaryEmail}`);
          } catch (emailError) {
            emailsFailed++;
            console.error(`   ⚠️ Failed to send email to ${customer.primaryEmail}:`, emailError.message);
